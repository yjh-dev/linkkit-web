        ...
        <div class="container mx-auto max-w-6xl">
            ...
            <div class="flex items-center justify-between mb-12">
                <div>
                    ...
                    <p class="text-gray-600">안녕하세요, <span
                            class="font-semibold text-linkkit-blue">{{ auth()->user()->name }}</span>님! 👋</p>
                </div>

                <div class="flex items-center gap-4">
                    <a href="/choose-preset"
                        class="bg-linkkit-blue hover:bg-blue-600 text-white px-6 py-3 rounded-xl font-semibold transition-all shadow-md hover:shadow-lg flex items-center gap-2">
                        <span>➕</span>
                        <span>새 페이지 만들기</span>
                    </a>
            ...
            <div class="bg-white rounded-2xl shadow-xl p-8">
                ...
                @if (auth()->user()->linkPages->count() > 0)
                    <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                        @foreach (auth()->user()->linkPages as $page)
                            <div
                                class="border-2 border-gray-200 rounded-2xl p-6 hover:border-linkkit-blue transition-all group">
                                <!-- 프리셋 뱃지 -->
                                <div class="flex items-center justify-between mb-4">
                                    <span
                                        class="px-3 py-1 rounded-full text-xs font-semibold
                                    @if ($page->preset === 'basic') bg-blue-100 text-blue-700
                                ...
                                <div class="flex gap-2">
                                    <a href="{{ route('linkpage.show', $page->uuid) }}" target="_blank"
                                    ...
                                    <a href="{{ route('linkpage.edit', $page->uuid) }}"
                                        ...
                                        {{-- ✨ 삭제 버튼 추가 --}}
                                <button onclick="confirmDelete('{{ $page->uuid }}', '{{ $page->name }}')"
                                    class="bg-red-100 hover:bg-red-200 text-red-600 p-2 rounded-xl transition-all">
                                ...
                                {{-- 삭제 폼 (숨김) --}}
                                <form id="delete-form-{{ $page->uuid }}"
                                    action="{{ route('linkpage.destroy', $page->uuid) }}" method="POST" class="hidden">
                                    @csrf
                                    @method('DELETE')
                                </form>
                            </div>
                ...
                @else
                    <div class="text-center py-12">
                        ...
                        <a href="/choose-preset"
                            class="inline-flex items-center gap-2 bg-linkkit-blue hover:bg-blue-600 text-white px-6 py-3 rounded-xl font-semibold transition-all">
                            <span>✨</span>
                            <span>페이지 만들기</span>
                        </a>
                    </div>
...
function confirmDelete(uuid, name) {
    if (confirm(`정말 "${name}" 페이지를 삭제하시겠습니까?\n\n이 작업은 되돌릴 수 없습니다.`)) {
